...
function authorAvatars(authorIds: number[]) {
  return authorIds.slice(0, 3).map(id => researchers.find(r => r.id === id)?.avatar ?? '??');
}

// ── Module-level mutable state ───────────────────────────────────────────────
let _pubs: Publication[] = initialPublications.map(p => ({ ...p, status: p.status as PubStatus }));
...
function usePubs() {
  ...
  return {
    pubs: _pubs,
    addPub: (p: Publication) => { _pubs = [..._pubs, p]; rerender(); },
    updatePub: (id: number, changes: Partial<Publication>) => {
      _pubs = _pubs.map(p => p.id === id ? { ...p, ...changes } : p);
      rerender();
    },
  };
}
...
function PubModal({
  ...
  const blank: Publication = {
    id: Date.now(),
    title: '', authors: [], type: 'Journal', venue: '',
    year: new Date().getFullYear(), status: 'Draft',
    citations: 0, doi: '', abstract: '', keywords: [], pages: '',
  };
  ...
  return (
    <Modal
      title={mode === 'add' ? 'Add Publication' : mode === 'edit' ? 'Edit Publication' : form.title}
      onClose={onClose}
      width={640}
      footer={!readOnly ? (
        <>
    ...
    >
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 12 }}>
        <div>
          ...
          <input className="field-input" value={form.title} onChange={e => set('title', e.target.value)} disabled={readOnly} style={{ background: readOnly ? '#F5F6F8' : undefined }} />
        </div>
        <div>
          <label className="field-label">Abstract</label>
          <textarea className="field-input" rows={3} value={form.abstract} onChange={e => set('abstract', e.target.value)} disabled={readOnly} style={{ resize: 'vertical', background: readOnly ? '#F5F6F8' : undefined }} />
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          ...
          <div>
            ...
            <select className="field-input" value={form.status} onChange={e => set('status', e.target.value as PubStatus)} disabled={readOnly} style={{ background: readOnly ? '#F5F6F8' : undefined }}>
              {['Draft', 'Submitted', 'Published', 'Archived'].map(s => <option key={s}>{s}</option>)}
            </select>
          </div>
          <div>
            <label className="field-label">Venue / Journal</label>
            <input className="field-input" value={form.venue} onChange={e => set('venue', e.target.value)} disabled={readOnly} style={{ background: readOnly ? '#F5F6F8' : undefined }} />
          </div>
          <div>
            <label className="field-label">Year</label>
            <input className="field-input" type="number" value={form.year} onChange={e => set('year', Number(e.target.value))} disabled={readOnly} style={{ background: readOnly ? '#F5F6F8' : undefined }} />
          </div>
          <div>
            <label className="field-label">DOI</label>
            <input className="field-input" value={form.doi} onChange={e => set('doi', e.target.value)} disabled={readOnly} style={{ background: readOnly ? '#F5F6F8' : undefined }} />
          </div>
          <div>
            <label className="field-label">Pages</label>
            <input className="field-input" value={form.pages} onChange={e => set('pages', e.target.value)} disabled={readOnly} style={{ background: readOnly ? '#F5F6F8' : undefined }} />
          </div>
        </div>
        <div>
          <label className="field-label">Keywords (comma-separated)</label>
          <input className="field-input" value={form.keywords.join(', ')} onChange={e => set('keywords', e.target.value.split(',').map(s => s.trim()).filter(Boolean))} disabled={readOnly} style={{ background: readOnly ? '#F5F6F8' : undefined }} />
        </div>
      </div>
    </Modal>
  );
...
function PublicationList({ onDetail }: { onDetail: () => void }) {
  const { role, can } = useRole();
  const isReviewer = role === 'Reviewer';
  const canWrite = can('add', 'publications');
  const { pubs, addPub, updatePub } = usePubs();
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 20, alignItems: 'center', flexWrap: 'wrap' }}>
        <div style={{ position: 'relative', flex: 1, minWidth: 220 }}>
          ...
          <input className="field-input" placeholder="Search publications…" value={search} onChange={e => setSearch(e.target.value)} style={{ paddingLeft: 32, height: 36, fontSize: 13 }} />
        </div>
        <div style={{ position: 'relative' }}>
          <select className="field-input" style={{ height: 36, fontSize: 13, paddingRight: 28, appearance: 'none', minWidth: 140 }} value={type} onChange={e => setType(e.target.value)}>
        ...
        {!isReviewer && (
          <div style={{ position: 'relative' }}>
            <select className="field-input" style={{ height: 36, fontSize: 13, paddingRight: 28, appearance: 'none', minWidth: 140 }} value={status} onChange={e => setStatus(e.target.value)}>
      ...
      <div className="card">
        <table className="data-table" style={{ width: '100%' }}>
          <thead>
            <tr>
              <th>Title</th><th>Authors</th><th>Type</th><th>Venue</th><th>Year</th><th>Status</th><th>Citations</th>
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={8} style={{ textAlign: 'center', padding: 32, color: '#9CA3AF' }}>
                {isReviewer ? 'No submissions awaiting review.' : 'No publications match your filters.'}
              </td></tr>
            ) : filtered.map(p => (
              <tr key={p.id}>
                ...
                <td>
                  <div style={{ display: 'flex' }}>
                    {authorAvatars(p.authors).map((av, i) => (
                      <div key={i} style={{ width: 26, height: 26, borderRadius: 6, background: '#16324F', color: '#fff', fontSize: 9, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', marginLeft: i > 0 ? -6 : 0, border: '1.5px solid #fff' }}>{av}</div>
                    ))}
                  </div>
                </td>
                <td><span style={{ padding: '2px 8px', background: '#F5F6F8', borderRadius: 4, fontSize: 12, color: '#5B6472', border: '1px solid #E1E4E8' }}>{p.type}</span></td>
                <td style={{ color: '#5B6472', fontSize: 13, maxWidth: 160, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.venue}</td>
                ...
                <td>
                  {isReviewer ? (
                    <ReviewActions
                      pub={p}
                      onApprove={() => updatePub(p.id, { status: 'Published' })}
                      onRequestChanges={() => updatePub(p.id, { status: 'Draft' })}
                    />
                  ) : (
                    <div style={{ display: 'flex', gap: 5 }}>
                      {canWrite && <button className="btn-secondary" style={{ padding: '4px 10px', fontSize: 12 }} onClick={() => setModal({ mode: 'edit', pub: p })}>Edit</button>}
                      <button className="btn-secondary" style={{ padding: '4px 10px', fontSize: 12 }} onClick={() => setModal({ mode: 'view', pub: p })}>View</button>
                    </div>
                  )}
                </td>
              </tr>
      ...
      {modal && (
        <PubModal
          ...
          onSave={p => {
            if (modal.mode === 'add') addPub(p);
            else updatePub(p.id, p);
          }}
        />
      )}
    </div>
...
export default function Publications() {
  ...
  return (
    <div>
      ...
      {sub === 'list' && <PublicationList onDetail={() => setSub('detail')} />}
      {sub === 'detail' && !isReviewer && <PublicationDetail onBack={() => setSub('list')} />}
      {sub === 'board' && !isReviewer && <PublicationBoard />}
    </div>
  );
}
