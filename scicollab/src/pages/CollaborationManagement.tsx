...
import { projects as storeProjects, researchers, Project } from '../data/store'
import { can, type Role } from '../permissions/config'
import Drawer from '../components/Drawer'
import FormModal, { inputCls, TagInput } from '../components/FormModal'
...
const coauthors: CoAuthor[] = [
  { name: 'Dr. Emma Torres', institution: 'Oxford', papers: 8, lastCollab: 'Oct 2024', status: 'Active' },
  { name: 'Prof. James Okafor', institution: 'Cambridge', papers: 5, lastCollab: 'Sep 2024', status: 'Active' },
  { name: 'Dr. Yuki Tanaka', institution: 'U. Tokyo', papers: 3, lastCollab: 'Aug 2024', status: 'Inactive' },
  { name: 'Dr. Amir Khan', institution: 'ETH Zürich', papers: 6, lastCollab: 'Nov 2024', status: 'Active' },
  { name: 'Dr. Priya Sharma', institution: 'IIT Bombay', papers: 2, lastCollab: 'Jul 2024', status: 'Active' },
]
...
const instCollabs: InstCollab[] = [
  {
    ...
    projects: 4,
    pubs: 14,
    leadContact: 'Dr. Emma Torres',
    email: 'e.torres@ox.ac.uk',
    focusAreas: ['Privacy-Preserving Computation', 'Federated Learning', 'Quantum Biology'],
    agreementStatus: 'Active',
    keyInitiatives: ['Joint PhD Fellowship Program', 'Cross-Silo Federated Medical Data Network', 'Annual AI & Ethics Symposium'],
  },
  {
    name: 'ETH Zürich',
    type: 'Joint Lab',
    ...
    projects: 3,
    pubs: 9,
    leadContact: 'Dr. Amir Khan',
    email: 'a.khan@ethz.ch',
    focusAreas: ['Secure Hardware', 'Zero-Knowledge Proofs', 'Cheminformatics'],
    agreementStatus: 'Active',
    keyInitiatives: ['ETH-MIT Secure AI Laboratory', 'Molecular Property Transformer Benchmark'],
  },
  {
    ...
    projects: 2,
    pubs: 5,
    leadContact: 'Dr. Yuki Tanaka',
    email: 'y.tanaka@u-tokyo.ac.jp',
    focusAreas: ['Climate Science', 'Neural Earth Emulation', 'Extreme Weather ML'],
    agreementStatus: 'Under Renewal',
    keyInitiatives: ['Global Climate-AI Early Warning Exchange', 'Pacific Environmental Computing Grant'],
  },
  {
    ...
    projects: 5,
    pubs: 18,
    leadContact: 'Prof. James Okafor',
    email: 'j.okafor@cam.ac.uk',
    focusAreas: ['Petascale Supercomputing', 'Distributed Graph Analytics', 'Exascale Simulation'],
    agreementStatus: 'Active',
    keyInitiatives: ['Exascale Graph Partitioning Infrastructure', 'High-Performance Drug Screening Cluster'],
  },
]

const initialTasks: ProjectTask[] = [
  { id: 1, projectId: 1, title: 'Finalize Differential Privacy bounds for FedGraph node updates', assignee: 'Dr. Sarah Chen', dueDate: '2026-08-30', status: 'In Progress', priority: 'High' },
  { id: 2, projectId: 1, title: 'Benchmark model latency across MIT and Oxford clusters', assignee: 'Dr. Emma Torres', dueDate: '2026-09-15', status: 'Pending', priority: 'Medium' },
  { id: 3, projectId: 1, title: 'Draft joint conference paper for NeurIPS 2026', assignee: 'Dr. Amir Khan', dueDate: '2026-10-01', status: 'Pending', priority: 'High' },
  { id: 4, projectId: 2, title: 'Simulate 49-qubit quantum coherence on photosynthetic complex', assignee: 'Dr. Emma Torres', dueDate: '2026-08-20', status: 'Completed', priority: 'High' },
  { id: 5, projectId: 2, title: 'Validate variational quantum eigensolver accuracy', assignee: 'Dr. Lena Kovacs', dueDate: '2026-09-10', status: 'In Progress', priority: 'Medium' },
  { id: 6, projectId: 3, title: 'Deploy 14-day atmospheric ensemble prediction pipeline', assignee: 'Dr. Yuki Tanaka', dueDate: '2026-08-25', status: 'In Progress', priority: 'High' },
  { id: 7, projectId: 4, title: 'Archive supercomputer log files and compound screening dataset', assignee: 'Prof. James Okafor', dueDate: '2026-07-30', status: 'Completed', priority: 'Low' },
  { id: 8, projectId: 5, title: 'Audit zero-knowledge verifier smart contract code', assignee: 'Dr. Amir Khan', dueDate: '2026-09-05', status: 'Pending', priority: 'Medium' },
]

function ResearcherDrawerContent({ researcher }: { researcher: typeof researchers[0] }) {
  return (
...
export default function CollaborationManagement({ role }: { role: string }) {
  ...
  const [localProjects, setLocalProjects] = useState<Project[]>(storeProjects)
  ...
  const handleUpdateProject = () => {
    ...
    setLocalProjects(prev => prev.map(p => p.id === editP.id ? editP : p))
    if (selectedProject?.id === editP.id) {
      setSelectedProject(editP)
    }
    setEditModalOpen(false)
    showToast(`Project "${editP.title}" updated`, 'success')
  }
  ...
  return (
    <div className="p-8 max-w-[1200px] mx-auto">
      ...
      {viewMode === 'network' && (
        <div className="bg-white rounded-2xl border border-[#E2E8F0] mb-6 shadow-sm" style={{ height: 520 }}>
          ...
          <div style={{ height: 'calc(520px - 60px)' }}>
            <NetworkGraph onNodeClick={(id) => {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            ...
            <div className="bg-white rounded-2xl border border-[#E2E8F0] p-5 shadow-sm">
              <div className="flex items-center justify-between mb-3">
                ...
                {can(typedRole, 'research.createCollaboration') && (
                  <button
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between p-4 bg-[#FAFAFA] rounded-xl border border-[#E2E8F0]">
              <div>
                ...
                <p className="text-xl font-bold text-[#0F172A]">
                  {tasks.filter(t => t.projectId === tasksProject.id).length}
                </p>
              </div>
              <div>
                <p className="text-xs text-[#64748B]">Completed</p>
                <p className="text-xl font-bold text-emerald-600">
                  {tasks.filter(t => t.projectId === tasksProject.id && t.status === 'Completed').length}
                </p>
              </div>
              <div>
                <p className="text-xs text-[#64748B]">In Progress</p>
                <p className="text-xl font-bold text-blue-600">
                  {tasks.filter(t => t.projectId === tasksProject.id && t.status === 'In Progress').length}
                </p>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-2.5">
                {tasks.filter(t => t.projectId === tasksProject.id).length === 0 ? (
